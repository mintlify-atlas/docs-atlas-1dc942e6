...
When you export an image or video frame, Betterflow captures the background as a separate step using **`modern-screenshot` (`domToCanvas`)** before compositing it with the foreground. This ensures CSS gradients, mesh backgrounds, blur filters, and noise overlays all render at full fidelity in the final export regardless of browser rendering quirks with canvas.

The compositing order is:

1. **Background layer** — captured via `domToCanvas`
2. **User image layer** — Konva stage export
3. **Overlay layer** — text and image overlays captured via `domToCanvas`
